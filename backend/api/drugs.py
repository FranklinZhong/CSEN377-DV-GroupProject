...
def drug_overview(
    ...
    # ── Fallback when FDA label is missing ───────────────────────────────────
    derived_source = False
    if not indication:
        # Try 1: benefit effects from effects table
        benefit_rows = conn.execute(
            "SELECT DISTINCT effect_name FROM effects "
            "WHERE drug_id = ? AND effect_type = 'benefit' "
            "ORDER BY frequency DESC LIMIT 5",
            (drug_id,),
        ).fetchall()
        if benefit_rows:
            names = ", ".join(r["effect_name"].replace("_", " ") for r in benefit_rows)
            indication = f"May support: {names}."
            derived_source = True
        else:
            # Try 2: body parts from side effects
            bp_rows = conn.execute(
                "SELECT DISTINCT body_part FROM effects "
                "WHERE drug_id = ? AND effect_type = 'side_effect' "
                "LIMIT 4",
                (drug_id,),
            ).fetchall()
            if bp_rows:
                parts = ", ".join(r["body_part"] for r in bp_rows)
                indication = f"Commonly affects: {parts} systems."
                derived_source = True
            else:
                indication = (
                    "No official FDA indication on file. "
                    "This may be a combination or specialty medication. "
    ...
    if derived_source:
        warnings.append(
            "Official FDA label not available. "
            "Showing information derived from clinical reports."
        )
    if not mechanism:
    ...
    source = "Derived from clinical reports" if derived_source else "FDA Drug Label"
