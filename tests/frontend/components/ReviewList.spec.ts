...
describe('ReviewList', () => {
  beforeEach(() => {
    ...
    api.getReviewsList.mockResolvedValue(mockResponse())
  })
  ...
  it('FE-RL-01: renders pagination when totalPages > 1', async () => {
    ...
    // Pager only renders if totalPages > 1
  ...
  it('FE-RL-02: calls api.getReviewsList on mount', async () => {
    ...
    mount(ReviewList, {
      props: { drugId: 1 },
      global: { stubs: { ElInput: true, ElSelect: true, ElOption: true, ElButton: true, ElSkeleton: true } },
    })
    await flushAsync()
    expect(api.getReviewsList).toHaveBeenCalledWith(1, expect.objectContaining({ page: 1 }))
  })

  it('FE-RL-03: shows empty message when no reviews', async () => {
    // @ts-ignore
    const { api } = require('@/api/client')
    api.getReviewsList.mockResolvedValue(mockResponse([], 0, 0))
    const wrapper = mount(ReviewList, {
      props: { drugId: 1 },
      global: { stubs: { ElInput: true, ElSelect: true, ElOption: true, ElButton: true, ElSkeleton: true } },
    })
  ...
  it('FE-RL-04: initialBodyPart prop passed to first API call', async () => {
    ...
    await flushAsync()
    expect(api.getReviewsList).toHaveBeenCalledWith(
      1,
